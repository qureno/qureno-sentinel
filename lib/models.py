...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, qurenod):
        golist = qurenod.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_qurenod(qurenod, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_qurenod(self, qurenod, rec):
        import decimal
        import qurenolib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from qurenod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from qurenod, with every run
        ...
        # get/create, then sync payment amounts, etc. from qurenod - Qurenod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(qurenod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from qurenod! %s" % e)
            govobj.vote_delete(qurenod)
            return (govobj, None)
    ...
    def vote_delete(self, qurenod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(qurenod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, qurenod, signal, outcome):
        import qurenolib
        ...
        output = qurenod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = qurenolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(qurenod, signal)

    def sync_network_vote(self, qurenod, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = qurenod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 qureno addr, non-multisig
            if not qurenolib.is_valid_qureno_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Qureno address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (qurenolib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not qurenolib.is_valid_qureno_address(addr, config.network):
    ...
    def hash(self):
        import qurenolib
        return qurenolib.hashit(self.serialise())
