...
def get_qureno_conf():
    if sys.platform == 'win32':
        qureno_conf = os.path.join(os.getenv('APPDATA'), "Qureno/qureno.conf")
    else:
        home = os.environ.get('HOME')

        qureno_conf = os.path.join(home, ".qureno/qureno.conf")
        if sys.platform == 'darwin':
            qureno_conf = os.path.join(home, "Library/Application Support/Qureno/qureno.conf")

    qureno_conf = sentinel_cfg.get('qureno_conf', qureno_conf)
