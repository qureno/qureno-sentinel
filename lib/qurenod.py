...
class QurenoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some qurenod methods
    ...
    @classmethod
    def from_qureno_conf(self, qureno_dot_conf):
        from qureno_config import QurenoConfig
        config_text = QurenoConfig.slurp_config_file(qureno_dot_conf)
        creds = QurenoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = qurenolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = qurenolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = qurenolib.blocks_to_seconds(diff)
